import { useState } from "react";

const CreateListing = () => {
  const [formData, setFormData] = useState({
    type: "rent",
    name: "",
    bedrooms: 1,
    bathrooms: 1,
    parking: "false",
    furnished: "false",
    address: "",
    description: "",
    offer: "false",
    regularPrice: 0,
    discountedPrice: 0,
  });
  const {
    type,
    name,
    bedrooms,
    bathrooms,
    parking,
    furnished,
    address,
    description,
    offer,
    regularPrice,
    discountedPrice,
  } = formData;

  const handleChange = () => {};
  return (
    <article className="max-w-md px-2 mx-auto">
      <h1 className="text-3xl text-center mt-6 font-bold">Create a Listing</h1>
      <form>
        <p className="text-lg mt-6 font-semibold">Sell / Rent</p>
        <div className="flex">
          <button
            type="button"
            id="type"
            className={`w-full px-7 py-3 font-medium text-sm uppercase shadow-md rounded hover:shadow-lg focus:shadow-lg active:shadow-lg transition duration-150 ease-in-out ${
              type === "rent"
                ? "bg-white text-black"
                : "bg-slate-600 text-white"
            }`}
            value="sale"
            onClick={handleChange}
          >
            sell
          </button>
          <button
            type="button"
            id="type"
            className={`w-full ml-3 px-7 py-3 font-medium text-sm uppercase shadow-md rounded hover:shadow-lg focus:shadow-lg active:shadow-lg transition duration-150 ease-in-out ${
              type === "sale"
                ? "bg-white text-black"
                : "bg-slate-600 text-white"
            }`}
            value="sale"
            onClick={handleChange}
          >
            rent
          </button>
        </div>
        <p className="text-lg mt-6 font-semibold">Name</p>
        <input
          type="text"
          id="name"
          placeholder="Name"
          className="w-full mb-6 px-4 py-2 text-lg text-gray-700 bg-white border border-gray-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white focus:border-slate-600"
          maxLength={32}
          minLength={10}
          value={name}
          onChange={handleChange}
          required
        />
        <div className="flex space-x-6 mb-6">
          <div>
            <p className="text-lg font-semibold">Beds</p>
            <input
              type="number"
              id="bedrooms"
              className="w-full px-4 py-2 text-xl text-center text-gray-700 bg-white border border-gray-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white focus:border-slate-600"
              min={1}
              max={50}
              value={bedrooms}
              onChange={handleChange}
              required
            />
          </div>
          <div>
            <p className="text-lg font-semibold">Bath</p>
            <input
              type="number"
              id="bathrooms"
              className="w-full px-4 py-2 text-xl text-center text-gray-700 bg-white border border-gray-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white focus:border-slate-600"
              min={1}
              max={50}
              value={bathrooms}
              onChange={handleChange}
              required
            />
          </div>
        </div>
        <p className="text-lg mt-6 font-semibold">Parking spot</p>
        <div className="flex">
          <button
            type="button"
            id="parking"
            className={`w-full px-7 py-3 font-medium text-sm uppercase shadow-md rounded hover:shadow-lg focus:shadow-lg active:shadow-lg transition duration-150 ease-in-out ${
              parking === "false"
                ? "bg-white text-black"
                : "bg-slate-600 text-white"
            }`}
            value={"true"}
            onClick={handleChange}
          >
            Yes
          </button>
          <button
            type="button"
            id="parking"
            className={`w-full ml-3 px-7 py-3 font-medium text-sm uppercase shadow-md rounded hover:shadow-lg focus:shadow-lg active:shadow-lg transition duration-150 ease-in-out ${
              parking === "true"
                ? "bg-white text-black"
                : "bg-slate-600 text-white"
            }`}
            value={"false"}
            onClick={handleChange}
          >
            no
          </button>
        </div>
        <p className="text-lg mt-6 font-semibold">Furnished</p>
        <div className="flex">
          <button
            type="button"
            id="furnished"
            className={`w-full px-7 py-3 font-medium text-sm uppercase shadow-md rounded hover:shadow-lg focus:shadow-lg active:shadow-lg transition duration-150 ease-in-out ${
              furnished === "false"
                ? "bg-white text-black"
                : "bg-slate-600 text-white"
            }`}
            value={"true"}
            onClick={handleChange}
          >
            yes
          </button>
          <button
            type="button"
            id="furnished"
            className={`w-full ml-3 px-7 py-3 font-medium text-sm uppercase shadow-md rounded hover:shadow-lg focus:shadow-lg active:shadow-lg transition duration-150 ease-in-out ${
              furnished === "true"
                ? "bg-white text-black"
                : "bg-slate-600 text-white"
            }`}
            value={"false"}
            onClick={handleChange}
          >
            no
          </button>
        </div>
        <p className="text-lg mt-6 font-semibold">Address</p>
        <textarea
          id="address"
          placeholder="Address"
          className="w-full mb-6 px-4 py-2 text-lg text-gray-700 bg-white border border-gray-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white focus:border-slate-600"
          value={address}
          onChange={handleChange}
          required
        />
        <p className="text-lg mt-6 font-semibold">Description</p>
        <textarea
          id="description"
          placeholder="Description"
          className="w-full mb-6 px-4 py-2 text-lg text-gray-700 bg-white border border-gray-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white focus:border-slate-600"
          value={description}
          onChange={handleChange}
          required
        />
        <p className="text-lg font-semibold">Offer</p>
        <div className="flex mb-6">
          <button
            type="button"
            id="offer"
            className={`w-full px-7 py-3 font-medium text-sm uppercase shadow-md rounded hover:shadow-lg focus:shadow-lg active:shadow-lg transition duration-150 ease-in-out ${
              offer === "false"
                ? "bg-white text-black"
                : "bg-slate-600 text-white"
            }`}
            value={"true"}
            onClick={handleChange}
          >
            yes
          </button>
          <button
            type="button"
            id="offer"
            className={`w-full ml-3 px-7 py-3 font-medium text-sm uppercase shadow-md rounded hover:shadow-lg focus:shadow-lg active:shadow-lg transition duration-150 ease-in-out ${
              offer === "true"
                ? "bg-white text-black"
                : "bg-slate-600 text-white"
            }`}
            value={"false"}
            onClick={handleChange}
          >
            no
          </button>
        </div>
        <div className="flex items-center mb-6">
          <div className="">
            <p className="text-lg font-semibold">Regular price</p>
            <div className="flex w-full justify-center items-center space-x-6">
              <input
                type="number"
                id="regularPrice"
                className="w-full px-4 py-2 text-xl text-center text-gray-700 bg-white border border-gray-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white focus:border=slate-600"
                min={50}
                max={5000000}
                value={regularPrice}
                onChange={handleChange}
                required
              />
              {type === "rent" && (
                <div className="">
                  <p className="w-full text-md whitespace-nowrap">$ / Month</p>
                </div>
              )}
            </div>
          </div>
        </div>
        {offer === "true" && (
          <div className="flex items-center mb-6">
            <div className="">
              <p className="text-lg font-semibold">Discounted price</p>
              <div className="flex w-full justify-center items-center space-x-6">
                <input
                  type="number"
                  id="discountedPrice"
                  className="w-full px-4 py-2 text-xl text-center text-gray-700 bg-white border border-gray-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white focus:border=slate-600"
                  min={50}
                  max={5000000}
                  value={discountedPrice}
                  onChange={handleChange}
                  required={offer === "true"}
                />
                {type === "rent" && (
                  <div className="">
                    <p className="w-full text-md whitespace-nowrap">
                      $ / Month
                    </p>
                  </div>
                )}
              </div>
            </div>
          </div>
        )}
        <div className="mb-6">
          <p className="text-lg font-semibold">Images</p>
          <p className="text-gray-600">
            The first image will be the cover (max 6)
          </p>
          <input
            type="file"
            id="images"
            className="w-full px-3 py-1.5 text-gray-700 bg-white border border-gray-300 cursor-pointer rounded transition duration-150 ease-in-out focus:bg-white focus:border-slate-600"
            accept=".jpg,.png,.jpeg"
            multiple
            onChange={handleChange}
            required
          />
        </div>
        <button
          type="submit"
          className="w-full mb-6 px-7 py-3 bg-blue-600 text-white font-medium text-sm uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out"
        >
          Create Listing
        </button>
      </form>
    </article>
  );
};

export default CreateListing;
